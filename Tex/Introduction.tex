...
本文从电动汽车动力系统的储能子系统和传动子系统两条主线入手，研究锂离子电池和旋转机械两大关键部件，分别针对前者的退化建模和后者的故障诊断两项关键问题展开研究。


面向安全苛刻工业场景，针对

设计并实现三种相应的数据驱动方法。
构建面向电动汽车动力系统关键子系统的~IPHM~方法体系。

围绕研究目标，本文将研究中心落在三个相互关联的挑战上，以兼顾退化建模的可信性和可泛化性，
以及故障诊断的幻觉鲁棒性。
关键研究挑战如下：

（1）\textbf{健康状态估计}

因此，如何精细表征不确定性、完备量化时空不确定性并阐明模型估计性能与不确定性量化质量间的关系，以提升模型可信性，
是本文重点攻克的核心问题。
...
其次，在可泛化~PHM~方面，针对。
再次，在抗幻觉~PHM~方面，针对。
最后，本文构建了电动汽车动力系统关键部件一体化~IPHM~原型，验证了所提方法的有效性、先进性和工程应用潜力。
基于此研究路线，本文的具体研究内容分为以下三个部分：

（1）\textbf{基于层次化随机时空Transformer的锂离子电池健康状态可信估计方法}
