...
% 
如图~\ref{fig:intro_UQ_framework}~所示，在贝叶斯方法的视角下，状态估计即学习从输入 $\boldsymbol{x}$ 到目标 $y$ 的条件预测分布 $p_{\boldsymbol{\theta}}(y|\boldsymbol{x})$，其中 $\boldmath{\theta}$ 是可学习的参数。传统确定性模型的预测分布因退化为零方差的狄拉克函数，即 $p_{\boldsymbol{\theta}}(y|\boldsymbol{x}) = \delta(y - f_{\boldsymbol{\theta}}(\boldsymbol{x}))$， 而表现出对预测结果的盲目置信，对未知工况时存在误判风险。对应地，概率性模型直接输出具备方差的连续概率密度函数，例如高斯分布，即 $p_{\boldsymbol{\theta}}(y|\boldsymbol{x}) = \mathcal{N}(\mu_{\boldsymbol{\theta}}(\boldsymbol{x}), \sigma^2_{\boldsymbol{\theta}}(\boldsymbol{x}))$，从而能够以方差 $\sigma^2_{\boldsymbol{\theta}}$ 对认知与偶然误差进行量化，在面对未见退化模式的样本时表达低置信度，赋予估计结果可信性。
%
...
目前，传统深度学习模型（Conventional Deep Learning Models, CDLMs）在智能化旋转机械故障诊断占据主流，但其针对大规模数据时模型容量和泛化能力有限，且仅能输出单一故障标签，难以提供归因分析与维修决策。LLMs~具备强泛化能力和可读诊断报告生成能力，在近年来获得广泛关注，图~\ref{fig:intro_LLM_framework}~展示了故障诊断基础模型的一般架构。
% 
