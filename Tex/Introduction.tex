...
动力电池因工况与个体差异导致的数据分布漂移削弱了退化模型的泛化性能。为实现可靠~PHM~对可泛化性的要求，现有文献依据对目标域数据的可访问性，发展出基于三种范式的退化建模方法，包括基于{预训练-微调范式}的退化建模方法，对应表中的\textbf{完全监督微调}与\textbf{半监督自适应}场景；基于{域自适应范式}的退化建模方法，主要对应表中\textbf{标准无监督自适应}场景；以及基于{域泛化范式}的退化建模方法，对应表中的\textbf{域泛化}场景。
