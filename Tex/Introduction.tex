...
三个部分的相关工作。
% 
这三个部分的探讨并不局限于单一研究对象，而致力于提炼相关研究的抽象机制。具体而言，本节内容的目的为探究如何在连续退化过程预测中引入不确定性量化，以满足场景对可信性的要求；研究如何消除跨工况或跨设备等导致的数据分布偏移，以实现可泛化退化建模；以及探索如何检测深度模型，尤其是大语言模型，在面对未知或边界样本时产生的盲目自信，从而赋予模型抗幻觉能力。
...
大语言模型具备较强的跨模态理解与生成能力，为复杂故障诊断提供新的技术路径，但其生成机制同时引入事实性错误风险，因此如何实现面向故障诊断场景的可靠幻觉检测与抑制，成为当前智能运维研究中的重要问题。一方面，训练数据中的类别不平衡与统计偏置容易使模型在推理过程中偏向高频状态，从而诱发对少数类故障的误判与幻觉生成。另一方面，在部署阶段，模型可能面对未见故障模式、跨设备输入或与任务无关的提示信息。若缺乏对自身认知边界的有效感知，模型便可能生成语言形式流畅但脱离事实依据的诊断结论。进一步来看，现有幻觉检测通常依赖词元级后验概率，而故障诊断中的幻觉更常表现为序列层面的局部事实错误。同时，大量语法正确但语义空泛的响应片段会掩盖真正的幻觉内容，使词元级不确定性与完整响应真实性之间产生显著的粒度失配。因此，如何揭示统计偏置、未见模式与幻觉生成之间的内在联系，建立从模型置信度到事实真值的一致性表征，并实现消除粒度偏差的序列级无偏不确定性量化，是大语言模型抗幻觉故障诊断的关键挑战。
...
% 
针对旋转机械故障诊断任务，研究重点在于复杂生成响应的不确定性量化，并提出序列级不确定性表征方法，将词元级不确定性与完整响应级不确定性进行对齐，以消除由粒度失配引起的偏差，进而实现面向故障诊断大模型的无偏幻觉检测。
% 
...
其次，提出一种序列级不确定性量化机制。该机制通过点双列相关分析与显著性检验定位关键词元，并设计数据驱动的关键词元不确定性重加权策略，充分对齐并消除词元级别与完整响应级别不确定性之间的粒度偏差，从而实现无偏的幻觉检测。
