...
面向电动汽车关键系统的~IPHM~模型需要具备高度可靠性，这在数据分布存在不一致的情况下包括三个方面：可信目标要求模型不仅能给出点估计结果，还能准确量化自身推断的认知边界与不确定性；可泛化目标要求模型在存在测试工况跨越与域偏移时，仍能保持稳健的特征挖掘与推断能力；抗幻觉指在引入大语言模型（Large Language Models, LLMs）等先进方法开展诊断或决策时，能够有效抑制模型在面对非期望输入时生成脱离物理事实的虚假输出。为满足上述可靠性目标，亟需在算法层面建立对未知分布特征认知、量化与利用的闭环机制。为此，本文引入了广义贝叶斯范式。基于该范式，本文分别针对动力电池的 SOH 评估任务、RUL 预测任务，以及旋转机械的故障诊断任务，提出了三个~IPHM~模型，旨在为实际车载工况下的数据分布漂移难题提供新颖的理论框架。
%
从现实应用角度看，本文研究同样具有多层面的重要意义。
% 
在安全保障层面，动力电池退化失准可能导致续航评估偏差、热失控风险累积以及寿命误判，旋转机械故障漏检或误诊则可能进一步诱发动力传动异常，影响整车运行稳定性，提升关键部件状态评估与故障识别的可靠性，是保障电动汽车长期安全服役的重要基础。
% 
在运维管理层面，具备可信不确定性量化能力和分布外鲁棒性的~IPHM~模型，有助于在复杂车载工况下提升健康管理系统对异常状态的提前感知能力，为预防性维护、剩余寿命评估、维护资源调度以及运行策略优化提供更可靠的依据，从而降低非计划停机和过度维修带来的成本损失。
% 
在产业发展层面，面向动力电池全生命周期管理的精确退化建模，不仅有助于支撑车载运行阶段的健康监测，也可为梯次利用、质保定价、残值评估和回收决策提供方法支撑；面向智能诊断场景的抗幻觉机制，则有助于推动大语言模型在安全关键工业场景中的稳健落地。
% 
本文研究对于提升电动汽车关键系统智能运维水平、支撑新能源汽车产业高质量发展、推动下一代高性能电动汽车的长效安全服役与规模化应用，具有重要的工程价值和现实意义。
...
近年来，数据驱动的电池~SOH~估计研究进一步向真实车载工况、早期寿命预测和部分充电片段建模等实际应用场景拓展。面向真实车载运行场景中高质量数据稀缺与多源观测异构的问题，文献\cite{liu2025multimodal_soh}中，Liu~等人构建了一个基于开放电动汽车数据的多模态~SOH~评估框架。该框架融合车辆运行数据、充电行为和统计健康特征，提升了模型在复杂实际工况下的~SOH~辨识能力。
文献\cite{che2025diagnostic_free}中，Che~等人提出了一种无需专门诊断工况和历史退化轨迹的车载电池健康评估方法。该方法利用正常充电或动态放电过程中的局部运行测量片段，并将差分电压分析等机理约束嵌入可解释机器学习模型中，实现了面向全寿命周期的快速健康诊断和退化轨迹重构。
文献\cite{hu2026batterygpt}中，Hu~等人提出了一种基于生成式预训练~Transformer~的锂离子电池早期退化预测方法，命名为~BatteryGPT。该方法利用早期充电数据自回归预测完整生命周期充电轨迹，并通过~SOH~估计器将预测轨迹与老化特征关联，从而在容量显著衰退发生前实现早期~SOH~和寿命趋势预测。
针对实际充电过程中仅能获取部分充电片段且深度模型可解释性不足的问题，文献\cite{li2026fourier_lstm}中，Li~等人提出了一种傅里叶信息增强的长短期记忆网络。该方法通过全局和局部频域滤波器提取多尺度频率特征，并结合因果~Shapley~加性解释（Causal Shapley Additive Explanations, Causal SHAP）分析解释特征贡献，实现了部分充电数据下兼具精度、鲁棒性和可解释性的~SOH~估计。
上述研究表明，SOH~估计模型正在由标准实验数据下的点估计任务转向真实应用场景下的可靠评估任务。然而，真实车载数据的工况扰动、观测缺失和分布差异会进一步放大模型推断偏差，因此仅提升预测精度仍不足以满足可靠决策需求，还需要通过概率建模或校准机制对估计结果的不确定性进行表征。
...
近年来，物理先验与神经网络的融合逐渐成为提升~PHM~模型泛化能力的重要方向。文献\cite{wang2026phynet_review}中，Wang~等人系统总结了物理模型、数据驱动模型和物理-神经网络融合模型在故障诊断与寿命预测中的作用，指出通过结构约束、损失约束和数据生成等方式引入物理知识，有助于缓解纯数据驱动模型在小样本、跨工况和可解释性方面的局限。
...
文献\cite{zhang2025batlinet}中，Zhang~等人针对不同循环协议、环境温度和电极材料引起的跨老化条件寿命预测问题，提出了一种基于电芯间学习机制的深度模型~BatLiNet。该方法通过同时学习单电芯退化信息和电芯间寿命差异，提升了模型在多样化老化条件和跨化学体系场景下的预测稳定性。
文献\cite{zhang2026discovery_learning}中，Zhang~等人提出了一种面向电池寿命预测的~Discovery Learning~框架。该框架将主动学习、物理引导学习和零样本学习整合到闭环推理流程中，能够从历史电池设计和极少量早期实验中预测新设计的循环寿命，体现了少样本实验条件下退化建模向物理知识驱动和科学机器学习融合发展的趋势。
...
%
文献\cite{zhou2026eis_dada}中，Zhou~等人提出了一种基于电化学阻抗谱（Electrochemical Impedance Spectroscopy, EIS）的无监督可迁移荷电状态估计方法，命名为~EIS-DADA。该方法首先基于等效电路模型提取具备物理可解释性的阻抗特征，继而在均值教师框架中引入局部对抗对齐和全局~MMD~对齐策略，从而缓解不同材料体系和健康状态间的特征分布差异。
...
文献\cite{yu2026lmm_fd}中，Yu~等人提出了一种面向机械故障诊断和智能运维的大规模多模态模型。该方法构建机械领域知识图谱以描述故障类型、传感信号和语义诊断知识之间的关系，并通过跨模态融合网络对齐时序振动信号与文本诊断描述，从而提升模型在故障诊断中的解释性和跨场景泛化能力。
文献\cite{lai2026fr_llm}中，Lai~等人提出了一种面向故障诊断和剩余寿命预测的多任务大语言模型，命名为~FR-LLM。该方法设计信号到文本编码机制以连接振动信号与~LLM~输入空间，并通过自适应多任务优化策略平衡分类诊断和寿命预测目标，提升了旋转机械退化场景下的诊断鲁棒性和任务协同性。
文献\cite{lin2026moe_llm}中，Lin~等人提出了一种基于混合专家（Mixture of Experts, MoE）大语言模型的多传感器柔性融合故障诊断方法。该方法将不同组合的振动、声学和电流等传感信号映射到统一的大模型特征嵌入空间，并通过稀疏激活的专家路由机制实现多源信息的自适应融合，有助于缓解复杂工业环境中传感器异构和信号缺失导致的诊断偏差。
面向更广义的智能运维闭环，文献\cite{di2026spacehmchat}中，Di~等人提出了一个面向航天器电源系统的全流程人机协同健康管理框架~SpaceHMchat。该框架覆盖工况识别、异常检测、故障定位和维修决策等任务，体现了大模型从单点诊断工具向可交互、可解释和闭环协同健康管理系统演化的趋势。
...
针对多源域数据不平衡导致的跨域诊断偏置问题，文献\cite{10944708}中，Bi~等人提出了一种双重重加权孪生对比学习网络，命名为~DRSC。该方法通过样本级和类别级重加权机制削弱源域类别不均衡对特征学习的影响，并利用孪生对比结构增强跨域共享故障表征的判别性，从而提升旋转机械在多源域不平衡场景下的诊断泛化能力。
...
\section{研究内容}\label{sec:Intro_research_content_and_approach}
...
该图展示全文的章节设置及其逻辑衔接关系，用于从整体层面说明本文的组织脉络。
全文共分为五章，具体内容安排如下：
