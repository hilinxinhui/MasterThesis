...
% 
文献\cite{20240715556058}中，Zhang~等人提出了一种无参考的、基于不确定性量化的幻觉检测方法，其核心是一种认知聚焦机制。具体地，这种机制在幻觉检测过程中首先聚焦于包含关键语义信息的核心 token；其次排除历史上下文中潜在的不可靠 toekn 从而阻断其可能诱发的级联幻觉效应；最后分析 token 类型和 token 频次等固有属性对生成结果可靠性的影响。
% 
...
大语言模型具备较强的跨模态理解与生成能力，为复杂故障诊断提供新的技术路径，但其生成机制同时引入事实性错误风险，因此如何实现面向故障诊断场景的可靠幻觉检测与抑制，成为当前智能运维研究中的重要问题。一方面，训练数据中的类别不平衡与统计偏置容易使模型在推理过程中偏向高频状态，从而诱发对少数类故障的误判与幻觉生成。另一方面，在部署阶段，模型可能面对未见故障模式、跨设备输入或与任务无关的提示信息。若缺乏对自身认知边界的有效感知，模型便可能生成语言形式流畅但脱离事实依据的诊断结论。进一步来看，现有幻觉检测通常依赖 token 级后验概率，而故障诊断中的幻觉更常表现为序列层面的局部事实错误。同时，大量语法正确但语义空泛的响应片段会掩盖真正的幻觉内容，使 toekn 级不确定性与完整响应真实性之间产生显著的粒度失配。因此，如何揭示统计偏置、未见模式与幻觉生成之间的内在联系，建立从模型置信度到事实真值的一致性表征，并实现消除粒度偏差的序列级无偏不确定性量化，是大语言模型抗幻觉故障诊断的关键挑战。
