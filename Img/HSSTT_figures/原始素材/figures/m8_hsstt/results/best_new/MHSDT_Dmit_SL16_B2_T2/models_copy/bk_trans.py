...
class AttentionBlock(nn.Module):
    def __init__(self, emb_dim, heads, droupout, forward_expansion):
...
class TransformerEncoder(nn.Module):
    def __init__(self, fea_dim, emb_dim, num_layers, heads, dropout=0.1, forward_expansion=2, device="cpu"):
        ...
        self.layers = nn.ModuleList(
            [AttentionBlock(emb_dim, heads, dropout, forward_expansion) for _ in range(num_layers)]
        )
...
if __name__ == "__main__":
    ...
    # input
    input = torch.rand(1, 16, 192).to(device)
    print(input.device)

    # model
    encoder = TransformerEncoder(fea_dim, emb_dim, num_layers, heads, dropout, forward_expansion, device).to(
        device
    )
