...
def posterior_predictive(path, config):
    ...
    samples = np.random.normal(mu, sigma * 0.5, size=(config["n_sample"], len(gts)))
    return samples.T
...
def get_data(config):
    assert config["data"] == "tongji", f"dataset mismatch"
    config["data_root_dir"] = "../../../data"
    config = argparse.Namespace(**config)
    data_module = TongjiDataset(config)
