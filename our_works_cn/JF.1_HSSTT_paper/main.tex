...
锂离子电池（LIBs）因其优越的特性而被广泛应用于工业领域\cite{NASAJPOURESFAHANI2024114783}。工业环境的苛刻条件，加上复杂的内部电化学反应， necessitate 增强措施以确保 LIBs 的安全可靠长期运行。因此，监测 LIBs 的性能以保证其持续运行至关重要，而电池性能通常用健康状态（SOH）表示，定义为电池当前最大可用容量与其额定容量的比值\cite{WANG2024110325, 10391265}。

深度学习的发展促进了数据驱动 SOH 估计模型的出现，这些模型由于减少对专家知识的依赖以及增强捕获退化机制复杂动态的能力，正逐步取代基于物理的模型\cite{mit_dataset_attia2020closed, tri_dataset_severson2019data, tongji_dataset_zhu2022data}。然而，数据驱动模型在面对具有复杂且不可访问分布的样本时仍然容易产生不准确的估计，这是由于工业环境中不可避免的噪声波动和可变负载需求所导致的\cite{10214203}。此外，这些模型的确定性性质限制了它们只能提供 SOH 的点估计，而无法发出错误信号，如图~\ref{fig:problem}(a) 所示。依赖此类确定性模型可能会危及后续的决策过程。例如，估计的 LIBs SOH 小于实际值可能导致过早报废，从而造成经济损失。相反，高估 SOH 可能导致过度循环，从而增加系统级热失控的潜在风险。

% 为缓解这些模型的无意识问题并确保工业监测和维护的可靠性，评估数据驱动预测模型输出的可信度至关重要。鉴于与可信 SOH 估计的重要关系，不确定性的作用被认为至关重要。一方面，不确定性作为一种度量，通过表征分布与真实值之间的差异来反映可信程度。另一方面，不确定性作为一个以用户为中心的指标，可以方便地集成到涉及人为干预的决策过程中。

为缓解确定性模型造成的问题并确保工业监测和维护的可靠性，评估数据驱动预测模型输出的可信度至关重要。鉴于与可信 SOH 估计的重要关系，不确定性的作用被认为至关重要\cite{THUY2024330, 10539360}。一方面，不确定性作为一种度量，通过表征分布与真实值之间的差异来反映可信程度。另一方面，不确定性作为一个以用户为中心的指标，可以方便地集成到涉及人为干预的决策过程中。现实世界的工业应用需要一个针对 LIBs 的可信预测模型，其面临的挑战概述如下。
...
% \begin{IEEEbiography}[{\includegraphics[width=1in,height=1.25in,clip,keepaspectratio]{biography/fig1}}]{林新辉} 于 2023 年获得华北电力大学智能科学与技术学士学位。他目前正在东北大学攻读计算机科学与技术硕士学位。
